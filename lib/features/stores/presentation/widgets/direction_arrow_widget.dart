...
class NavigationArrowWidget extends ConsumerStatefulWidget {
  ...
  final StoreEntity? selectedShop;
  ...
  @override
  ConsumerState<NavigationArrowWidget> createState() =>
      _NavigationArrowWidgetState();
}
...
class _NavigationArrowWidgetState extends ConsumerState<NavigationArrowWidget>
  ...
  Offset? _calculatePositionOnRoute(BuildContext context) {
    ...
    try {
      ...
      LatLng closestPoint = widget.routePoints.reduce(
        (a, b) =>
            _calculateDistance(currentLatLng, a) <
                    _calculateDistance(currentLatLng, b)
                ? a
                : b,
      );
      ...
      final mapRenderBox = renderObject;
      ...
      final visibleBounds = _estimateVisibleBounds(
        mapRenderBox,
        widget.mapController,
      );

      final xRatio =
          (closestPoint.longitude - visibleBounds.west) /
          (visibleBounds.east - visibleBounds.west);
      final yRatio =
          1 -
          (closestPoint.latitude - visibleBounds.south) /
              (visibleBounds.north - visibleBounds.south);

      return Offset(xRatio * mapSize.width, yRatio * mapSize.height);
    } catch (e) {
  ...
  void _showArrivalNotification() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Vous êtes arrivé à ${widget.selectedShop?.name}'),
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      left:
          routePosition.dx -
          30, // Centrer la flèche (30 = moitié de la largeur)
      top:
          routePosition.dy -
          30, // Centrer la flèche (30 = moitié de la hauteur)
      child: Material(
