/// Page de vérification OTP migrée vers Clean Architecture.
library;

// ============================================================
// MUKHLISS - Page Vérification OTP (Migrée)
// ============================================================
...
class _OtpVerificationPageState extends ConsumerState<OtpVerificationPage>
  ...
  void initState() {
    ...
    AppLogger.navigation(
      'OtpVerificationPage initialisée pour ${widget.email}',
    );
  ...
  /// ✅ Vérification OTP avec le nouveau système
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      if (widget.type == OtpVerificationType.passwordReset) {
        AppLogger.auth('Vérification OTP reset password');

        // ✅ Activer le flag AVANT la vérification pour bloquer la redirection auto
        ...
        // Signup flow
        AppLogger.auth('Vérification OTP signup');
        ...
        final verifyResult = await authClient.verifyOtp(
          email: widget.email,
          token: _otpCode,
          isRecovery: false, // ✅ Signup
        );

        await verifyResult.when(
          success: (_) async {
            // ✅ Compléter l'inscription avec completeSignup
            // L'utilisateur a été créé par signInWithOtp, on complète ses données
            final signupResult = await authClient.completeSignup(
              email: widget.email,
            ...
            signupResult.when(
              success: (_) {
                if (mounted) {
                  showSuccessSnackbar(
                    context: context,
                    message: l10n?.signupSuccess ?? 'Inscription réussie!',
                  );
      ...
      AppLogger.auth('Erreur inattendue OTP', level: LogLevel.error, error: e);
  ...
  /// ✅ Renvoyer OTP avec le nouveau système
  Future<void> _resendOtp() async {
    ...
    try {
      ...
      AppLogger.auth('Renvoi OTP pour ${widget.email}');
      final result = await authClient.sendOtp(widget.email);

      result.when(
        success: (_) {
          if (mounted) {
            _showSuccessSnackbar(
              l10n?.nouveaucodeenvoye ?? 'Nouveau code envoyé',
            );
          }
        },
        failure: (error) {
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(
                isExpired
                    ? l10n?.codeexpire ?? 'Code expiré'
                    : l10n?.codeexpiredans ?? 'Code expire dans',
                style: TextStyle(
