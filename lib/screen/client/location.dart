...
enum MapLayerType { plan, satellite, terrain, trafic }
...
class _LocationScreenState extends ConsumerState<LocationScreen>
  ...
  final int _searchResultsPerPage =
      10; // Nombre d'éléments à charger à chaque fois
  ...
  void _searchStores(String query) {
    ...
    setState(() {
      if (query.isEmpty) {
        _searchResults =
            stores; // Affiche tous les magasins si la recherche est vide
      } else {
        _searchResults =
            stores
                .where(
                  (store) =>
                      store.nom_enseigne.toLowerCase().contains(
                        query.toLowerCase(),
                      ) ||
                      store.adresse.toLowerCase().contains(
                        query.toLowerCase(),
                      ) ||
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Itinéraire calculé vers ${shop.nom_enseigne} (${_getModeName(_selectedMode)})',
            ),
                  ...
                  child:
                      icon != null
                          ? Icon(
                            ...
                            size: 30,
                            color:
                                isSelected ? Colors.blue.shade700 : iconcolor,
                          )
                          : const SizedBox(),
                ),
              ),
    ...
    return Scaffold(
      body: Stack(
        children: [
                ...
                _buildControlButton(
                  ...
                  onPressed:
                      _currentPosition != null
                          ...
                          : null,
                  backgroundColor:
                      isDarkMode ? AppColors.darkSurface : AppColors.accent,
                ),
                const SizedBox(height: 12),
                _buildControlButton(
                  icon: Icons.refresh,
          ...
          if (_showTransportModes) _buildBottomSheet(),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.alt_route, color: Colors.blue.shade700, size: 20),
              const SizedBox(width: 8),
              Text(
              ...
              children: [
                Expanded(
                  child: _buildInfoItem(
                    ...
                    OSRMRoutingService().formatDistance(
                      _routeInfo!['distance'].toDouble(),
                    ),
                ...
                Expanded(
                  child: _buildInfoItem(
                    Icons.access_time,
                    l10n?.duree ?? 'Durée',
                    OSRMRoutingService().formatDuration(
                      _routeInfo!['duration'].toDouble(),
                    ),
                    Colors.green.shade700,
                  ),
                ),
  ...
  Future<void> _showShopDetails(Store shop) async {
    ...
    final themeMode = ref.watch(themeProvider);
    final isDarkMode =
        themeMode == AppThemeMode.light; // Correction du dark mode
    final currentLocale = ref.watch(languageProvider);
    ...
    if (clientId == null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            l10n?.connectionRequired ?? 'Connectez-vous pour voir vos points',
          ),
    ...
    try {
      if (clientId != null) {
        clientMagazin = await ref
            .read(clientMagazinServiceProvider)
  ...
  void _showSearchBottomSheet() async {
    ...
    setState(() {
      ...
      _visibleSearchResults =
          _searchResultsPerPage; // Réinitialiser à chaque ouverture
    });
